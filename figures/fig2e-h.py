...
# %%
scatter_df['PMID'] = scatter_df['term'].apply(lambda x: x.split('-')[0])
scatter_df['Kinase'] = scatter_df['term'].apply(lambda x: x.split('_')[-1])

# %%
# Display Scatter Plots
plot_pmid = get_scatterplot(scatter_df,color_by='PMID', hide_legend=True)
# display(HTML(f"<div style='font-size:1.5rem;'>Scatter plot visualization for Rummagene Kinases (by PMID).</div>"))
# show(plot_pmid)
# output_file(filename=f"Figures/Rummagene_Kinases_byPMID.html")
# save(plot_pmid)
export_svg(plot_pmid, filename=str(fig_dir/"2h.svg"))

# %%
# Display Scatter Plots
plot_top20 = get_scatterplot(scatter_df,color_by='Kinase', limit=20)
# show(plot_top20)
# output_file(filename=f"Figures/Kinases_byKinaseTop20.html")
# save(plot_top20)
export_svg(plot_top20, filename=str(fig_dir/"2g.svg"))
