...
@click.option('--commercial', type=bool, default=False)
def update_background(enrich_url, commercial):
  ...
  new_background, = plpy.cursor('''
    insert into app_public_v2.background (gene_ids, n_gene_ids, commercial)
    select
  ...
  if commercial:
      new_background, = plpy.cursor('''
        ...
        from (
          ...
          inner join app_public_v2.gene_set_pmc pmc on gs.id = pmc.id
          where pmc.license = any({'CC BY', 'CC0'})
        ) gs, jsonb_each(gs.gene_ids) gsg(gene_id, nil)
        returning id;
      ''')
  plpy.conn.commit()
