...
def import_paper_info(plpy):
  ...
  oa_file_list = pd.read_csv('https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_file_list.csv', usecols=['Accession ID', 'License'], index_col='Accession ID')
  ...
  if title_dict:
    copy_from_records(
      plpy.conn, 'app_public_v2.pmc_info', ('pmcid', 'yr', 'doi', 'title', 'license'),
      tqdm((
        dict(
          pmcid=pmc,
