...
export default function Stats({
  ...
  if (show_gene_sets) {
    ...
    return (data?.pmcs?.totalCount !== undefined ? <><span className={classNames({'font-bold': bold})}>{Intl.NumberFormat("en-US", {}).format(data.pmcs.totalCount)}</span>&nbsp; GEO studies</> : <span className='loading'>loading</span>)
  } else if (show_publications) {
    return (data?.pmcStats?.nPublicationsProcessed !== undefined ? <><span className={classNames({'font-bold': bold})}>{Intl.NumberFormat("en-US", {}).format(data.pmcStats.nPublicationsProcessed)}</span>&nbsp;GEO studies</> : <span className='loading'>loading</span>) 
